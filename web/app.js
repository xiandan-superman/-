...
downloadCsv.addEventListener("click", () => {
  ...
  link.href = url;
  link.download = `${dateDimension.value}_${countrySelect.value}_summary.csv`;
  link.click();
...
function configureDimensionFilters(mapping) {
  ...
  populateDimensionSelect(countrySelect, state.options.country, ["总体", "US"]);
  populateDimensionSelect(platformSelect, state.options.platform, ["总体"]);
  populateDimensionSelect(channelSelect, state.options.channel, ["总体"]);
...
function populateDimensionSelect(select, values, defaults) {
  ...
  select.value = defaultValue || ALL_FILTER_VALUE;
}
...
function filterRecords(records) {
  const selected = {
    country: state.dimensionEnabled.country ? countrySelect.value || ALL_FILTER_VALUE : ALL_FILTER_VALUE,
    platform: state.dimensionEnabled.platform ? platformSelect.value || ALL_FILTER_VALUE : ALL_FILTER_VALUE,
    channel: state.dimensionEnabled.channel ? channelSelect.value || ALL_FILTER_VALUE : ALL_FILTER_VALUE,
  };
...
function filterPieRecords(records) {
  const selected = {
    country: state.dimensionEnabled.country ? pieCountrySelect.value || ALL_FILTER_VALUE : ALL_FILTER_VALUE,
    platform: state.dimensionEnabled.platform ? piePlatformSelect.value || ALL_FILTER_VALUE : ALL_FILTER_VALUE,
    channel: state.dimensionEnabled.channel ? pieChannelSelect.value || ALL_FILTER_VALUE : ALL_FILTER_VALUE,
  };
  return records.filter((record) => {
    if (!matchesDimension(record.country, selected.country)) return false;
    if (!matchesDimension(record.platform, selected.platform)) return false;
...
function matchesDimension(recordValue, selectedValue) {
  if (selectedValue === ALL_FILTER_VALUE) return true;
  return normalizeValue(recordValue) === normalizeValue(selectedValue);
}

function getFilterTitle() {
  const parts = [];
  if (state.dimensionEnabled.country) parts.push(displayFilterValue(countrySelect.value));
  if (state.dimensionEnabled.platform) parts.push(displayFilterValue(platformSelect.value));
  if (state.dimensionEnabled.channel) parts.push(displayFilterValue(channelSelect.value));
  return parts.length ? parts.join(" / ") : "总体";
}

function displayFilterValue(value) {
  return value === ALL_FILTER_VALUE ? "不过滤" : value || "不过滤";
}
